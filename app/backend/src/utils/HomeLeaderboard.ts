...
export const scoreBoard = (each: ITeam, home: IHomeBoard, away: IHomeBoard) => {
  const createBoard = {
    name: each.teamName,
  ...
  const efficiency = Math.round(
    (createBoard.totalPoints * 10000) / (createBoard.totalGames * 3),
  ) / 100;

  const finalBoard = { ...createBoard, efficiency };
  return finalBoard;
};
